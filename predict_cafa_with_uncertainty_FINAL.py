#!/usr/bin/env python3
"""
CAFA Protein Function Prediction with Uncertainty Quantification - FINAL VERSION

This is the final version that uses corrected threshold logic throughout.
This script evaluates PFP models (trained on SAR data) on CAFA preprocessed data
to assess out-of-distribution performance. It uses three evaluation approaches:
...
def load_cafa_data(ontology_name: str, cafa_dir: str, logger: logging.Logger) -> Tuple[np.ndarray, List[List[str]]]:
    ...
    # Construct file path based on ontology mapping
    ontology_mapping = {
        'process': 'BPO',
        'component': 'CCO', 
        'function': 'MFO'
    ...
    # Load the tuples (tm_vec, go_terms)
    ...
    for tm_vec, go_terms in cafa_data:
        if tm_vec is not None:  # Ensure embedding is available
...
def filter_cafa_data_by_pfp_vocab(go_terms_list: List[List[str]], embeddings: np.ndarray, 
    ...
    for i, go_terms in enumerate(go_terms_list):
        # Filter terms to only those in PFP vocabulary
        filtered_terms = [term for term in go_terms if term in pfp_vocab]
        
        if go_terms:  # Original protein had terms
            proteins_with_terms += 1
        
        # Only keep proteins that have at least one term after filtering
        if filtered_terms:
...
def create_tsv_like_data(go_terms_list: List[List[str]], embeddings: np.ndarray) -> Tuple[pd.DataFrame, List]:
    """Create TSV-like data structure from CAFA GO terms for evaluation compatibility."""
    
    # Create dummy dataframe similar to process_GO_data output
    data = []
    for i, go_terms in enumerate(go_terms_list):
        for go_term in go_terms:
            data.append({
                'Entry': f'protein_{i}',  # Use consistent protein IDs
                'GO_term': go_term
            })
...
def evaluate_predictions_cafa(median_probs: np.ndarray, mad_uncertainty: np.ndarray, 
    ...
    """Evaluate predictions against CAFA ground truth using three approaches."""
...
def save_results(ontology: str, median_probs: np.ndarray, mad_uncertainty: np.ndarray,
    ...
    # Save predicted terms dictionary
...
def main():
    ...
    # FDR threshold options (using same defaults as predict_with_uncertainty_and_thresholds.py)
